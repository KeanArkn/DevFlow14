...
const LocalSearchbar = ({route, iconPosition, imgSrc, placeholder, otherClasses} : CustomInputProps) => {
  return (
    <div className={`background-light800_darkgradient flex min-h-[56px] grow items-center gap-4 rounded-[10px] px-4 ${otherClasses}`}>
        {iconPosition === 'left' && ( <Image src={imgSrc} alt='local search' width={24} height={24} className='cursor-pointer' /> )}
        ...
        {iconPosition === 'right' && ( <Image src={imgSrc} alt='local search' width={24} height={24} className='cursor-pointer' /> )}
    </div>
  )
}
